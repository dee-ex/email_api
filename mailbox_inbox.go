...
func HandleGetAllInbox(w http.ResponseWriter, r *http.Request) {
	var data LoginUser
...
func HandleGetDetailInbox(w http.ResponseWriter, r *http.Request) {
	var data LoginUser
    err := json.NewDecoder(r.Body).Decode(&data)
    if err != nil {
        http.Error(w, err.Error(), 400)
    ...
    mail, err := GetDetailInbox(c, uint32(seqid))
    json.NewEncoder(w).Encode(mail)
}

func GetAllInbox(c *client.Client) ([]Envelope, error) {
